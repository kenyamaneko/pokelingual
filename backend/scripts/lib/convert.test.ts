...
describe("[ポケモンデータ] ポケモンレコードへの変換", () => {
  ...
  ])("$expected.name_ja を変換すると、英名・和名・種族値合計・タイプ・説明・バージョン別説明一覧・レベルアップ技一覧が出力される", ({ species, pokemon, levelUpMoves, expected }) => {
    expect(convertToPokemonRecord(species, pokemon, levelUpMoves)).toMatchObject(expected);
  });
