...
describe("採点単位からのスコア算出", () => {
  it("単位が1件で判定値が0.61のとき、スコアは61になる", () => {
    expect(computeScoreFromUnits([0.61])).toBe(61);
  });

  it("判定値が1.0, 0.6, 0.2の3件のとき、平均の60がスコアになる", () => {
    expect(computeScoreFromUnits([1.0, 0.6, 0.2])).toBe(60);
  });
