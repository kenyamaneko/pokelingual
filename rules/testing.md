...
- 共通ルールの「本番の実データ ID をそのまま使わない」は本リポでは適用しない。図鑑番号・名前・タイプ・種族値・説明文を含め、公開済みのポケモンデータはゲームの仕様として後から変わることが実質的に無いため、テストでも実データをそのまま使ってよい
  - 例外: ポケモン名の伏せ字化 (`maskPokemonNameEN`/`JA` 等) のように、説明文にそのポケモンの名前が含まれる前提で動作を確かめるテストでは、実データの説明文には対象ポケモンの名前が通常含まれない (PokeAPI の flavor text は自身の名前を書かないことが多い) ため、名前を含む文を意図的に作って使ってよい
