  ...
  if (pokemon.length === 0) {
    return (
      <div className="text-center py-20">
        <p className="text-gray-400 text-lg mb-2">まだ ポケモンに であっていません</p>
        <p className="text-gray-400 text-sm">クエストに ちょうせんして ポケモンを みつけよう！</p>
      </div>
    );
  }
    ...
    <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-3">
      {pokemon.map((p) => (
        ...
        >
          ...
          {p.status !== "captured" && (
            <span className="text-[10px] text-gray-400">はっけんずみ</span>
          )}
        </button>
      ))}
    </div>
