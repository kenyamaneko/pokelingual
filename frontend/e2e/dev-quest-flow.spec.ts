...
test("チュートリアル未完了のとき、チュートリアルを完了すると本番クエストへの導線に切り替わる", async ({
  ...
  // チュートリアルは固定シナリオなので、内容を確定的に検証できる
...
test("チュートリアル完了済みのとき、クエストを一周すると図鑑まで到達できる", async ({ page }) => {
  ...
  // 採点結果。実 Gemini の採点レイテンシに採点演出の順次表示が続くため、既定の 5 秒では
  // 最後に出る「ダメージ」表示に間に合わないことがある。mock 版ヘルパの待ち時間に合わせる。
  await expect(page.getByText(TEXT.damage)).toBeVisible({ timeout: 30000 });
