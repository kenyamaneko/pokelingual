...
push → CI 再実行 → バックエンドデプロイ → デプロイ後スモーク
                   フロントエンドデプロイ・テスト観点カタログ/カバレッジ公開（並行）
```
...
| ファイル | トリガー | 内容 |
|---------|---------|------|
| `ci.yml` | PR, workflow_call | バックエンド/フロントエンドテスト・lint・型チェック、E2E、Terraform fmt、テスト観点カタログの job summary 出力 |
| `deploy-dev.yml` | push to `main` | CI 再実行 → dev デプロイ → スモーク → テスト観点カタログ/カバレッジ公開 |
