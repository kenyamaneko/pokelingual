...
- Cloud Run のレスポンス時間が異常に速い場合、devmock が動いている可能性がある
...
バックエンドが devmock モードで動いていた（上記 #1 の問題）。
devmock の `GetRandomPokemon` は固定のテストデータを返すが、条件によっては空の description になる場合がある。
...
- devmock が意図せず本番で動いているケースは、レスポンス時間で見抜ける
