...
prod の Firestore `(default)` データベースに Point-in-Time Recovery (PITR) を有効化する。`google_firestore_database` リソースに属性を1つ追加するだけで、過去7日以内の任意時点への復元が可能になる。復元は同じ `(default)` データベースに対する機能のため、復元後のデータ移行や接続先切替も不要。dev は使い捨てデータのため対象外にする。
