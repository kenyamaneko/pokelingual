...
- **月次予算**：Billing のレポートでサービス別の内訳を確認する (想定上の主要因は Vertex AI)。アプリ層のレートリミット (1ユーザー 30回/日・全体 1,500回/日、ADR-011) が上限装置なので、上限に達していないのに予算を超過するペースなら単価が想定とずれている。次節で上限を下げる。
