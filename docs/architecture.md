...
backend と frontend は独立して順にデプロイするため（backend → frontend）、一時的に backend だけ先のバージョンに進む。frontend が取り残されても不具合が起きないよう、backend は常に直近の frontend と後方互換を保ち、契約を壊す変更は複数リリースに分けて出す（ADR-016）。
